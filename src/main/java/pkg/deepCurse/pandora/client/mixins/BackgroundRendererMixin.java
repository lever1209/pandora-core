...
public class BackgroundRendererMixin {
	...
	private static Vec3d pandora_overrideFog(DimensionEffects effects, Vec3d color, float f, Operation<Vec3d> operation,
		...
		var settings = ClientConfig.CLIENT.DimensionSettings.get(world.getDimensionKey().getValue());

		var result = operation.call(effects, color, f);

		if (settings != null) {
			return new Vec3d(result.x * settings.fogLevel * ClientEndTickCallback.EffectStrength,
					result.y * settings.fogLevel * ClientEndTickCallback.EffectStrength,
					result.z * settings.fogLevel * ClientEndTickCallback.EffectStrength);
		}
